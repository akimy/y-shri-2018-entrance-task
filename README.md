...
Подробно об архитектуре. Я использую реакт для построения SPA приложений. Есть несколько паттернов и хороших практик, которые  применяются в реакт сообществе. В частности, разделение компонентов на "components" и "containers". Первые представляют собой чаще всего чистые функции, не хранят в себе состояние и отвечают только за отображение данных. Вторые - контейнеры. Они отвечают за получение данных, обработку данных и отправку их компонентам-представлениям. Именно они будут темой этого задания. Таким образом директория /client/components - это именно то, что Вас интересует. Остальному мы тоже уделим немного внимания. Тестов написанно не было, во первых - я плох в написании тестов. Во вторых на это просто недостаточно времени, я еще не завершил второе задание до конца. Однако, тут была использованна зависимость prop-types для валидации всех типов входящих данных в компонент. Это позволяет избежать ошибок связанных с ложными сравнениями, приведением типов, да и вообще динамической нетипизированной природой javascript. В случае если в компонент придет props отличающийся от задекларированного - браузер сообщит нам в консоли об ошибке и предоставит исчерпывающую информацию о ней.
