...
4. Проверяем запросы в GraphiQL согласно схеме, запрос на events не работает, т.к. название переменной arguments в resolvers/query,js намеренно изменено. Заменяем ее обратно на args.